...
| **Live Stats** | Post-submission brief strip: shift rank, shift average, shift highest, ahead/behind counts |
| **Analysis** | Full analysis screen: shift rank card, shift vs. shift comparisons, score histogram, radar chart, subject comparison bars, stream donut |
...
| `drawArcGauge(svgEl, fillEl, ratio)` | Computes SVG path data for a semicircular arc gauge from 0 to `ratio` (0–1). Used for the main score gauge. |
...
Reads only the single shift's stats node. Computes students above/same/below, and calls `renderBriefStrip` to inject the HTML strip into `#liveStatsBrief`.
...
- Computes user's median, mean within their shift
...
- Shift Rank card (showing rank, total students, and Top X%)
...
- The `scoreCounts` histogram allows score distribution computation without storing individual scores.
