...
window.escapeHtml = window.sanitizeHTML;
...
function checkStoredSession() {
  try {
    ...
    if (meta) {
      ...
      filename.textContent = session.filename || 'Unknown file';
      const mode = document.createElement('strong');
      mode.textContent = session.examMode || 'PCM';
      const lastOpened = document.createElement('strong');
      lastOpened.textContent = timeStr;
      meta.replaceChildren(
        filename,
        document.createElement('br'),
        document.createTextNode('Mode: '),
        mode,
        document.createTextNode(` - ${session.questions.length} questions`),
        document.createElement('br'),
        document.createTextNode('Last opened: '),
        lastOpened
      );
    }
...
function collectPDFData() {
  return {
    st: computeStats(questions),
    fname: document.getElementById('topbarFile').textContent || 'Unknown File',
    mode: examMode,
...
function buildPDFLayout(data) {
  const { jsPDF } = window.jspdf;
  const doc   = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
  const { st, fname, mode, date } = data;
...
function triggerPDFDownload(doc, mode) {
  doc.save(`ExamReport_${mode}_${Date.now()}.pdf`);
}

function exportPDF() {
  if (!questions.length) {
    alert('No data to export. Please upload a response sheet first.');
    return;
  }
  if (!window.jspdf) {
  ...
  const doc = buildPDFLayout(data);
  triggerPDFDownload(doc, data.mode);
}
...
function setMode(m) {
  ...
  const predictorBanner = document.getElementById('predictorCtaBanner');
  if (predictorBanner) {
    predictorBanner.style.display = m === 'PCB' ? 'none' : 'flex';
  }
...
// core parser
function stripPortalWatermarks(text) {
  const wmFreq = {};
  const wmRe = /\b(\d{4,10})\b/g;
  let wmMatch = wmRe.exec(text);
  while (wmMatch !== null) {
    wmFreq[wmMatch[1]] = (wmFreq[wmMatch[1]] || 0) + 1;
    wmMatch = wmRe.exec(text);
  }
  ...
  if (watermarks.length > 0) {
    ...
    return text.replace(wmPattern, '');
  }
  return text;
}

function updateOldSheetFlag(text) {
  try {
...
function getPortalSection(questionIndex) {
  if (questionIndex < 50) return 'Physics';
  if (questionIndex < 100) return 'Chemistry';
  return examMode === 'PCB' ? 'Biology' : 'Mathematics';
}

function getPortalOptionLabels(block) {
  const optionIds = [...new Set(
    [...block.matchAll(/\b(3\d{5})\b/g)].map(match => match[1])
  )].sort((a, b) => parseInt(a) - parseInt(b));

  return optionIds.reduce((labels, id, index) => {
    labels[id] = ['A', 'B', 'C', 'D'][index] || String(index + 1);
    return labels;
  }, {});
}

function parsePortalText(rawText) {
  const text = stripPortalWatermarks(rawText);
  updateOldSheetFlag(text);
  ...
  for (let i = 0; i < corrMatches.length; i++) {
    ...
    const section = getPortalSection(qs.length);
    const idToLabel = getPortalOptionLabels(block);
...
async function loadDash(filename, qs) {
  ...
  // Offline shift signature check
  if (window.STATIC_SHIFT_SIGNATURES && signature) {
    const matchingShift = window.STATIC_SHIFT_SIGNATURES[signature];
    if (matchingShift) {
      if (matchingShift.stream !== examMode || matchingShift.attempt !== selectedAttempt || matchingShift.shift !== selectedShift) {
        if (typeof _showWrongShiftPopup === 'function') {
          _showWrongShiftPopup(matchingShift.shift, qs, st, filename, signature, matchingShift.stream, matchingShift.attempt);
        } else {
          alert('Wrong shift detected. This sheet belongs to ' + matchingShift.shift + ' (' + matchingShift.stream + ' - ' + matchingShift.attempt + '). Please select the correct shift.');
...
function renderDashboard(qs) {
  ...
  document.getElementById('metricsGrid').innerHTML = `
    <div class="metric-card" data-tooltip="out of ${sanitizeHTML(st.maxM)} total marks">
      <div class="metric-val mv-blue">${sanitizeHTML(st.earned)}<span style="font-size:15px;font-weight:400;color:var(--text3);font-family:var(--font-body)">/${sanitizeHTML(st.maxM)}</span></div>
      <div class="metric-lbl">Total Score</div>
    </div>
    <div class="metric-card" data-tooltip="out of ${sanitizeHTML(qs.length)} questions">
      <div class="metric-val mv-green">${sanitizeHTML(st.correct)}</div>
      <div class="metric-lbl">Correct</div>
    </div>
    <div class="metric-card" data-tooltip="out of ${sanitizeHTML(qs.length)} questions">
      <div class="metric-val mv-red">${sanitizeHTML(st.incorrect)}</div>
      <div class="metric-lbl">Incorrect</div>
    </div>
    <div class="metric-card" data-tooltip="based on attempted questions only">
      <div class="metric-val mv-amber">${sanitizeHTML(st.accuracy)}%</div>
  ...
  document.getElementById('subjectBars').innerHTML = st.subStats.map(s => `
    <div class="bar-row">
      <div class="bar-rowlbl">${sanitizeHTML(s.s.substring(0, 5))}</div>
      <div class="bar-track"><div class="bar-fill" style="width:${sanitizeHTML(s.pct)}%;background:${SC[s.s] || '#00ff88'}"></div></div>
      <div class="bar-pct">${sanitizeHTML(s.e)}/${sanitizeHTML(s.mx)}</div>
    </div>`).join('');
...
function renderGrid(qs) {
  ...
  document.getElementById('gridContainer').innerHTML = ordered.map(s => {
    ...
    return `<div class="subject-group">
      <div class="subject-label">${sanitizeHTML(s)}</div>
      <div class="q-grid">${sq.map(q => {
        const status = ['correct', 'incorrect', 'unattempted'].includes(q.status)
          ? q.status
          : 'unattempted';
        return `<button class="q-btn ${status}" id="qbtn-${Number(q.id) - 1}"
          onclick="jumpToQ(${Number(q.id) - 1});closeSidebar()"
          title="${sanitizeHTML(s)} Q${sanitizeHTML(q.sectionNum)}: ${status}">${sanitizeHTML(q.sectionNum)}</button>`;
      }).join('')}</div>
    </div>`;
  }).join('');
}
...
function updateQuestionUI(q, prefix) {
  ...
  badge.className = 'q-badge ' + (status === 'correct' ? 'badge-correct' : status === 'incorrect' ? 'badge-incorrect' : 'badge-unattempted');
  ...
  const img = sanitizeImageURL(questionImages[q.id]);
  imgArea.replaceChildren();
  if (img) {
    const preview = document.createElement('img');
    preview.src = img;
    preview.alt = `Q${q.id}`;
    preview.className = 'zoomable-img';
    preview.title = 'Click to zoom';
    if (isQd) preview.style.cursor = 'zoom-in';
    imgArea.appendChild(preview);
  } else {
    const placeholder = document.createElement('div');
    placeholder.className = 'q-img-placeholder';
    placeholder.appendChild(document.createTextNode('Question image not available'));
    placeholder.appendChild(document.createElement('br'));
    const hint = document.createElement('small');
    hint.style.cssText = 'font-size:11px;margin-top:4px;display:block';
    hint.textContent = 'Please upload the PDF version of your response sheet for question previews.';
...
function showQuestion(idx, scroll) {
  ...
  document.querySelectorAll('.q-btn.active').forEach(b => {
...
function switchScoreTab(tabName, btn) {
  ...
  document.querySelectorAll('.score-tab-panel').forEach(p => {
...
function renderScoreCard(st) {
  ...
  document.getElementById('subjectScoreRows').innerHTML = st.subStats.map(s => `
    <div class="subject-score-row" data-subject="${sanitizeHTML(s.s)}">
      <span class="subject-score-name">${sanitizeHTML(s.s)}</span>
      <span class="subject-score-earned">${sanitizeHTML(s.e)}</span>
      <span class="subject-score-max">/ ${sanitizeHTML(s.mx)}</span>
    </div>`).join('');
}
...
function renderQuestionTable() {
  ...
  tbody.innerHTML = filteredQs.map(q => {
    const status = ['correct', 'incorrect', 'unattempted'].includes(q.status)
      ? q.status
      : 'unattempted';
    const statusIcon  = status === 'correct' ? '✓' : status === 'incorrect' ? '✗' : '—';
    const statusClass = `q-table-status q-table-status--${status}`;
    ...
    const previewHtml = img
      ? `<img class="q-table-thumb" src="${sanitizeHTML(img)}" alt="Q${sanitizeHTML(q.id)}" loading="lazy">`
      : `<div class="q-table-no-thumb">—</div>`;
    return `<tr data-qid="${sanitizeHTML(q.id)}" onclick="onQTableRowClick(${Number(q.id) - 1})">
      <td>${sanitizeHTML(q.id)}</td>
      <td style="font-family:var(--font-mono);font-size:11px;color:var(--text2)">${sanitizeHTML(String(q.section).substring(0,3).toUpperCase())}-${sanitizeHTML(q.sectionNum)}</td>
      <td class="q-table-preview-col">${previewHtml}</td>
      <td><span class="${statusClass}">${sanitizeHTML(statusIcon)}</span></td>
      <td><span class="${marksClass}">${sanitizeHTML(marksText)}</span></td>
    </tr>`;
  }).join('');
...
function highlightQTableRow(qId) {
  const tbody = document.getElementById('qTableBody');
  if (!tbody) return;
  tbody.querySelectorAll('tr.q-table-active').forEach(r => {
    r.classList.remove('q-table-active');
  });
...
function resetApp() {
  ...
  if (predictorOverlay) {
    ...
    predictorOverlay.setAttribute('inert', '');
  }
