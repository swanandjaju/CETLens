...
function populatePredictorAttempts(rows) {
  ...
  defaultOption.textContent = attempts.length ? 'Select attempt...' : 'No live attempts yet';
  select.appendChild(defaultOption);
  
  attempts.forEach(attempt => {
    const opt = document.createElement('option');
    opt.value = attempt;
    opt.textContent = attempt;
...
function populatePredictorShifts(attempt) {
  ...
  defaultOption.disabled = true;
  defaultOption.selected = true;
  defaultOption.textContent = attempt ? (shifts.length ? 'Select shift...' : 'No live shifts for attempt') : 'Select attempt first';
  select.appendChild(defaultOption);
